import { createGalleryService, getGalleryService, updateGalleryService } from "../service/galleryService.js";

export const createGalleryController = async (req, res) => {
  const galleryData = await createGalleryService(req.body);
  return res.status(200).json({ message: "Gallery created", data: galleryData });
};
...
export const getGalleryByIdController = async (req, res) => {
  const galleryId = parseInt(req.params.galleryId);
  const galleryData = await getGalleryByIdController(galleryId);
  return res.status(200).json({ message: "gallery by id data", data: galleryData });
};
...
export const updateGalleryStatusController = async(req,res)=>{
    ...
    return(gallery? res.status(201).json({message:"gallery status updated",gallery}):res.status(404).json({message:"gallery status not updated",gallery}))
} 